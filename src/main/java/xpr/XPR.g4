...
save_statement |if_statement |cond_expression  ;
...
validate_num:  VNUM   ID_GLOBAL  COMMA cond_num   all_result
;
cond_num: VGE | VGT  | VLE | VLT;

belong_statement: BELONG  lvalue
;
has_many_statement: HAS_MANY  lvalue (del_expr)? ;
del_expr:  COMMA DEP DESTROY   ;
...
all_result : ( int_result | float_result |  nil_t| string_result | dynamic_result | global_result | bool_t ( expression)?);
...
primary_expr: int_result | int_plus | int_minus | new_statement  |function_call | lvalue;
...
id_function : ID DOT ID | (SELF DOT)? (ID | ID_FUNCTION);
